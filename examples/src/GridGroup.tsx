  ...
  return gridRefs.map((ref, _index) => (
      <Grid
        ...
        onFocusedStateChange={(sourceIndex: number, columnIndex: number, rowIndex: number) => {
          gridRefs.filter((ref, index) => index !== _index).forEach(ref => ref.current?.refreshFocusedState(sourceIndex, columnIndex, rowIndex));
        }}
        onSelectedStateChange={(sourceIndex:number, columnIndex: number, rowIndex: number) => {
          gridRefs.filter((ref, index) => index !== _index).forEach(ref => ref.current?.refreshSelectedState(sourceIndex, columnIndex, rowIndex));
        }}
        onViewportStateChange={(sourceIndex: number) => {
          gridRefs.filter((ref, index) => index !== _index).forEach(ref => ref.current?.refreshViewportState(sourceIndex));
        }}
      />
  ));
}
